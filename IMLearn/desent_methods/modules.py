...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        m = len(y)
        # return - (1/m) * np.sum(y[i] * (X[i] @ self.weights) - np.log(1+np.exp(X[i] @ self.weights))
        #                         for i in range(m))
        return - (1/m) * np.sum((y.reshape(-1, 1) * X) @ self.weights - np.log(1+np.exp(X @ self.weights)))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        # return - (1/m) * np.sum(y[i] * X[i] - (1/(1+np.exp(X[i] @ self.weights))) * np.exp(X[i] @ self.weights) * X[i]
        #                         for i in range(m))
        return - (1/m) * (np.sum(y.reshape(-1,1) * X
                                 - ((1/(1+np.exp(X @ self.weights))) * np.exp(X @ self.weights)).reshape(-1, 1) * X,
                                 axis=0))
